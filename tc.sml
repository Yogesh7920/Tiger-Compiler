...
val default = not(pp) andalso not(ast) andalso not(ir) andalso not(cflag) andalso not(bb)
...
val tree = if (ir orelse cflag orelse bb orelse default) then Translate.compile program else Tree.EXP(Tree.CONST 0)
